...
export function TutorialOverlay({ forceShow = false, onClose }: { forceShow?: boolean, onClose?: () => void }) {
  ...
  useEffect(() => {
    ...
    const hasSeenTutorial = safeGetItem('mergeverse_tutorial_v4', '');
    if (!hasSeenTutorial) {
      setIsVisible(true);
    }
  }, [forceShow]);
  ...
  const completeTutorial = () => {
    setIsVisible(false);
    if (!forceShow) {
      safeSetItem('mergeverse_tutorial_v4', 'true');
    }
    if (onClose) onClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-[5000] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md pointer-events-auto">
      <motion.div 
      ...
      >
        <div className="relative p-6 px-8 bg-gradient-to-br from-fuchsia-900/40 to-cyan-900/40 border-b border-white/10">
          <button onClick={completeTutorial} className="absolute top-4 right-4 p-2 bg-white/10 rounded-full text-slate-300 hover:text-white transition-colors">
            <X className="w-5 h-5" />
          </button>
        ...
        <div className="p-6">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <p className="text-slate-300 text-sm leading-relaxed text-center font-medium mt-2 min-h-[80px]">
